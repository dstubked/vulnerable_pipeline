node {
    ...
    stage('Push image') {
        ...
        /* scan image using scanner CLI */
            ...
            sh "hostname"
            sh "docker run -v /var/run/docker.sock:/var/run/docker.sock registry.aquasec.com/scanner:4.2 scan -H http://http://aquasec-demo658-vm0.eastus.cloudapp.azure.com:8080 -U jenkins_scanner -P P@ssword --registry "Docker Hub" docker-test:latest --html"
    }
}
